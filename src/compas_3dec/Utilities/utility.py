...
def displacement_concave(x_dis, y_dis, z_dis, n_of_regions, step, equ_time, cycle, d_vector, region, equil_check=False, block_centroid_result=True, contact=False):
    # generate step name
    step_name = 'x' + "_" + str(x_dis * step) + 'mm' + "_" + 'y' + "_" + \
        str(y_dis * step) + 'mm' + "_" + 'z' + "_" + str(z_dis * step) + 'mm'
    header = ';____________________________________________________________________' + '\n'
    header += ';_____DISPLACEMENT_____' + " " + \
        'Step' + '_' + str(step) + '___' + step_name + '\n'
    header += ';____________________________________________________________________' + '\n'
    ...
    if block_centroid_result:
        text = save_blocks_output(
            'step' + '_' + '0' + str(step) + '___' + step_name)
        header += text
        header += 2 * '\n'

    if contact:
        text1 = save_contacts_output('step' + '_contact_' + '0' +
                             str(step) + '___' + step_name)
        header += text1
        header += 2 * '\n'
    ...
    if block_centroid_result:
        text2 = save_blocks_output(
            'step' + '_' + '0' + str(step) + '_equ__' + step_name)
        header += text2
        header += 2 * '\n'

    if contact:
        text3 = save_contacts_output('step' + '_contact_equ_' +
                             '0' + str(step) + '__' + step_name)
        header += text3
        header += 2 * '\n'
...
def displacement_file(writepath, n_steps, displ_x, displ_y, displ_z, n_region, equ_time, cycle, d_vector, title, region):
    ...
    for i in disp_step:
        main_string += displacement_concave(displ_x, displ_y, displ_z, n_region, i, equ_time, cycle, d_vector,region, True, True, True)
        main_string += 2 * '\n'
    # save file
