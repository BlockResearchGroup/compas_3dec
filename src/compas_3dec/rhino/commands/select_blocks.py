...
def from_rhino_select(group_types=["support", "other_group"], group_colors=[(255, 0, 0), (255, 0, 255)]):
        ...
        # ==============================================================
        mesh_supports_guids = rs.GetObjects("select closed valid meshes for the overall simulation", preselect=True, select=True, group=False, filter=rs.filter.mesh)
        ...
        for i in range(len(group_types)): # user can accidentally do not select anything
            ...
            mesh_supports_guids = rs.GetObjects("select_"+group_types[i], preselect=True, select=True, group=False, filter=rs.filter.mesh)
            rs.UnselectAllObjects()
